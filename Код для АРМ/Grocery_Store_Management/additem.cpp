...
void AddItem::on_btnAdd_clicked()
{
    ...
    query.clear();
    query.prepare("insert into database(Id, Name, Quantity, Status, DOP, DOS, SellingPrice, PurchasePrice) values ('"+sId +"','" +sName +"','" +sQuantity + "','"+ sStatus + "','" +sDOP + "','"+ sDOS+ "','"+sSellingPrice+ "','"+ sPurchasePrice + "')");

    if(!query.exec())
    {
        qDebug() << query.lastError().text() << query.lastQuery();
        ui->lblInfo->setText("Невозможно добавить");
    }
    else
    {
        if(query.numRowsAffected() > 0)
        {
            ...
            ui->lblInfo->setText("Продукт добавлен!");
            resetElements();
        }
        else
